...
export function maintenanceImpactEmail({ name, action, booking, maintenanceReason }) {
  const heading = action === "CANCELLED"
    ? "Your Booking Was Cancelled Due to Maintenance"
    : "Your Booking Was Reassigned Due to Maintenance";

  const body = action === "CANCELLED"
    ? `<p>Unfortunately, your booking has been <strong>cancelled</strong> because the room requires maintenance during your scheduled dates.</p>`
    : `<p>Your booking has been <strong>reassigned to a different room/bed</strong> because the original room requires maintenance during your scheduled dates.</p>`;
